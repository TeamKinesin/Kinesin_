...
namespace StellaRabbitStudio
{
	public class TMPVFXDrawer : TMPro.EditorUtilities.TMP_SDFShaderGUI
	{
		...
		public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
		{
			...
			{
				DrawEffect("Paper Noise", "PAPER_NOISE_ON", new EffectPropertyRange(68, 73, false), true);
				HandDrawn("Hand Drawn", "HANDDRAWN_ON");
				DrawEffect("Black Hole", "BLACKHOLE_ON", new EffectPropertyRange(82, 85, false), true);
				DrawEffect("Sketch Outline", "SKETCHOUTLINE_ON", new EffectPropertyRange(87, 93, false), true);
				DrawEffect("Text Box", "TEXTBOX_ON", new EffectPropertyRange(95, 106, false), true);
				DrawEffect("Scribble Noise", "SCRIBBLENOISE_ON", new EffectPropertyRange(108, 112, false), true);
				DrawEffect("Doodle", "DOODLE_ON", new EffectPropertyRange(114, 119, false), true);
				DrawEffect("Distortion", "DISTORTION_ON", new EffectPropertyRange(121, 125, false), true);
			}
		...
		private bool DrawEffectHeader(string inspector, string keyword, bool currentToggle, bool isPropertyToggle = false)
		{
			...
			effectNameLabel.tooltip = keyword + " shader property";
		...
		private void DrawProperty(int index, bool noReset = false)
		{
			...
			EditorGUILayout.BeginHorizontal();
			{
				...
				propertyLabel.tooltip = targetProperty.name + " shader property";
		...
		private bool DrawEffectSubKeywordToggle(string inspector, string keyword, bool setCustomConfigAfter = false)
		{
			...
			propertyLabel.tooltip = keyword + " shader property";
		...
		private void HandDrawn(string inspector, string keyword)
		{
			...
			if (toggle)
			{
				EditorGUILayout.BeginVertical(propertiesStyle);
				{
					MaterialProperty randomOffsetProp = ShaderGUI.FindProperty("_HandDrawnRandomEnabled", matProperties);
					
					EditorGUILayout.BeginHorizontal();
					matEditor.ShaderProperty(randomOffsetProp, new GUIContent("Use Random Offset"));
					EditorGUILayout.EndHorizontal();
					
					if (randomOffsetProp.floatValue > 0.5f)
					{
						DrawProperty(77);
						DrawProperty(78);
						DrawProperty(79);
