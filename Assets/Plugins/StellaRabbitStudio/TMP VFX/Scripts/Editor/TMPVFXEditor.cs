...
namespace StellaRabbitStudio
{
    ...
    public class TMPVFXEditor : UnityEditor.Editor
    {
        ...
        // 이펙트 정의를 위한 Dictionary
        private readonly Dictionary<string, (System.Type type, string name)> effectDefinitions = new()
        {
            { "_HANDDRAWNEnabled", (typeof(HandDrawnOnce), "HandDrawn") },
            { "_SCRIBBLENOISEEnabled", (typeof(ScribbleNoiseOnce), "ScribbleNoise") },
            { "_BLACKHOLEEnabled", (typeof(BlackHoleOnce), "BlackHole") },
            { "_DOODLEEnabled", (typeof(DoodleOnce), "Doodle") }
        };
        ...
        private void AddEffectComponents()
        {
            ...
            {
                if (material.HasProperty(effect.Key))
                {
                    if (material.GetFloat(effect.Key) > 0.5f)
                    {
                        var component = tmpvfx.GetComponent(effect.Value.type) as BaseEffectController;
                        if (component == null)
                        {
                            component = tmpvfx.gameObject.AddComponent(effect.Value.type) as BaseEffectController;
                            component.Awake();
                            component.LoadMaterialProperties();
                            Debug.Log($"TMPVFX: Successfully added {effect.Value.name} component");
                        }
                        else
                        {
                            component.Awake();
                            component.LoadMaterialProperties();
                            Debug.Log($"TMPVFX: Updated existing {effect.Value.name} component");
                        }
                        effectAdded = true;
                    }
                }
